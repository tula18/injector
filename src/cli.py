...
# from securestegcli.decoder import Decoder
# from securestegcli.metadata import Metadata
...
def main():
    ...
    if args.command == 'encode':
        encoder = Encoder(args.input_file, args.image_file, args.password, args.security_levels, logger)
        encoder.encrypt_and_embed()
    elif args.command == 'decode':
        logger.info(f"{Fore.CYAN}Starting the decoding process{Style.RESET_ALL}")
        # decoder = Decoder(args.image_file, args.password, logger)
        # decoder.extract_and_decrypt()
    elif args.command == 'info':
        logger.info(f"{Fore.CYAN}Retrieving metadata information{Style.RESET_ALL}")
        # metadata = Metadata(args.image_file, args.password, logger)
        # metadata.get_info()
